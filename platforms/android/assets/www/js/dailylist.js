...
Note.create = function( values ) {
	...
	if( null != returnObj ) { return returnObj; }
...
var  Event = Note.extend(
	'Event', {
		...
		,"construct": function( values ) {
			...
			if( 'undefined' != typeof values.status ) { this.status = values.status; }
			if( 'undefined' != typeof values.periodicity ) { this.periodicity = values.periodicity; }
			}

		,"getBullet": function() { return Event.BULLET; }
		,"renderEditPage": function( toolbarDom, editingDom ) {
			toolbarDom
				.prepend($('<div class="button" name="done"></div>').on(
					'click',
					function() {
						editingDom.find('.bulletList .bulletIcon[class~="selected"]').detach();
						}))
				.prepend($('<div class="button" name="fastForward"></div>').on(
					'click',
					function() {
						editingDom
...
Note.registerNoteType(
	'Event', Event,
	Event.BULLET,
	function(values) { if( 'event' == values.type ) { return new Event( values ); } return null; },
	10 );
...
var  Task = Note.extend(
	'Task', {
		...
		,"construct": function( values ) {
			...
			if( 'undefined' != typeof values.status ) { this.status = values.status; }
			if( 'undefined' != typeof values.modifier ) { this.modifier = values.modifier; }
			if( 'undefined' != typeof values.periodicity ) { this.periodicity = values.periodicity; }
			}

		,"getBullet": function() { return Task.BULLET; }
	});
Task.BULLET = '&#x2022;';
Note.registerNoteType(
	'Task', Task,
	Task.BULLET,
	function(values) { if( 'task' == values.type ) { return new Task( values ); } return null; },
	20 );
...
var  Thought = Note.extend(
	'Thought', {
		...
		,"getBullet": function() { return Thought.BULLET; }
	});
Thought.BULLET = '&#x002D;';
Note.registerNoteType(
	'Thought', Thought,
	Thought.BULLET,
	function(values) { if( 'thought' == values.type ) { return new Thought( values ); } return null; },
	30 );
...
var  Spending = Note.extend(
	'Spending', {
		...
		,"getBullet": function() { return Spending.BULLET; }
	});
Spending.BULLET = '$';
Note.registerNoteType(
	'Spending', Spending,
	Spending.BULLET,
	function(values) { if( 'spending' == values.type ) { return new Spending( values ); } return null; },
	40 );
...
var Provider = {
	...
	,"create": function( values ) {
        ...
        if( null == aItems ) { aItems = []; }

        var lastIdx = aItems.length;
        aItems.forEach( function( elem ) {
        	if( 'number' == typeof elem.id && elem.id > lastIdx ) { lastIdx = elem.id; }
        	});
	...
	,"delete": function( itemId ) {
		...
		while( idx >= 0 && 'undefined' != aItems[idx].id && itemId != aItems[idx].id ) { --idx; }

		var values = null;
		if( idx >= 0 ) { values = aItems.splice(idx,1); }
		...
		if( null != values ) {
	        ...
	        this.queryCache.forEach(function(itemCache) {
	        	if( itemCache.conds(values) ) {
	        		var idx = itemCache.array.indexOf(function(itemValues) { return (itemValues.id==values.itemId); });
	...
	,"read": function( conds, list ) {
        ...
        if( 'undefined' == typeof list || null == list || 'function' != typeof list.add ) { return itemsMatched; }
...
console.log('Show '+ this.id);
			if( "" == this.id ) { this.id = 'date-'+Date.today().getTime(); }  // Timestamp
...
//					,"background-color": "transparent"
			...
			if( 'object' == typeof this.data && 'function' == typeof this.data.isDataReceived ) {
				this.data.on('add',    function(ev) { thisObj.dataCreateListener(ev); } );
				this.data.on('remove', function(ev) { thisObj.dataDeleteListener(ev); } );
				this.data.on('change', function(ev) { thisObj.dataUpdateListener(ev); } );
						...
						setTimeout(
							function() {
								thisObj.data.forEach(function(item) {
									thisObj.dataCreateListener( {"items": {"inserted": [ item ]}} );
									});							
								}, 50 );
						}
					else {
						var contentDiv = this.dom.find('.listContent');
						var state      = contentDiv.attr('class').replace(/listContent/g, '').trim();
...
*/
		,"updateTitle": function() {
	        ...
	        templateDay = templateDay.replace( /##DAY##/g, '<sub>'+today.toString('ddd').substr(0,1).toUpperCase()+'</sub>'+today.toString('dd/MM') );
	        pageDOM.append( templateDay );
	        $('.header').text("- "+today.toString('MMM').substr(0,1).toUpperCase()+today.toString('MMM').substr(1).toLowerCase()+'. '+today.toString('yyyy')+' -');
	        var newTitle = '';

	        DailyListApp.updateTitle( 'list' );
			}  //  END (NoteList).updateTitle()

		,"show": function( animation ) {
			this.updateTitle();
			if( 0 < this.dom.siblings('.currentDayList').length ) {
				if( 'increase' == animation ) {
					...
					this.dom.removeClass('list').css('transform', 'rotateY(-90deg) skewY(-3deg)').show().addClass('list');
					}
				else {
					transformValue = '';
					transformDom = $();
				...
				var domObj = this.dom;
				setTimeout(function () {
					transformDom.find('.header').detach();
					transformDom.css({
						 "transform-origin": "0% 0% 0px"
						,"transform": "rotateY(0deg) skewY(0deg)"
						,"position": ""
				...
				this.dom.addClass('currentDayList').show();
				}
		...
		,"dataCreateListener": function( event ) {
			...
			itemTemplate.match(/##\w+##/g).forEach(function(elem) {
				var fieldName = elem.replace(/#/g,'').toLowerCase();
				...
				if( 0 < fieldName.indexOf('_') ) { fieldName.match(/_\w/g).forEach(function(character) { fieldNameCapitalized=fieldNameCapitalized.replace(character, character.toUpperCase()); }); }
				if( 'undefined' != typeof note[fieldNameCapitalized] ) {
					itemTemplate = itemTemplate.replace( elem, note[fieldNameCapitalized] );
					return;
					}
				if( 'function' == typeof note['get'+fieldNameCapitalized.substr(0,1).toUpperCase()+fieldNameCapitalized.substr(1)] ) {
					itemTemplate = itemTemplate.replace( elem, note['get'+fieldNameCapitalized.substr(0,1).toUpperCase()+fieldNameCapitalized.substr(1)] );
					return;
					}
				});
			...
			if( backgroundDom.length <= 2 ) {
				backgroundDom.first().before( domItem );
			}
			else {
				backgroundDom.first().replaceWith( domItem );
				}

			// Remove comments:
			domItem = $( domItem ).map( function(elem) { if( 1 != this.nodeType ) { return null; } return this; } );

			this.trigger( 'insert', {"inserted":[domItem]} );
			}
		...
		,"adjustBackground": function() {
		    ...
		    var contentItems = 1+this.dom.find('.item').length;
		    ...
		    while( lines < 20 && contentItems != Math.floor( totalHeight/lineHeight ) ) {
		    	++lines;
		        if( contentItems < Math.floor( totalHeight/lineHeight ) ) {
		        	var newItem = $('<div></div>').addClass('itemList').addClass('itemBackground');
		            ...
		            contentDom.find('.itemBackground').first().remove();
		            --contentItems;
		            }
		        }  //  END while
...
var  DailyList = NoteList.extend(
	'DailyList', {
		...
		,"construct" : function( dataArray, domParent, listDate ) {
			if( 'undefined' == typeof date || null == date ) { var i = 1/0; }
		...
		,"updateTitle": function() {
	        var newTitle = ''+this.date.toString('MMM').substr(0,1).toUpperCase()+this.date.toString('MMM').substr(1).toLowerCase()+'. '+this.date.toString('yyyy');

	        DailyListApp.updateTitle( newTitle );
			}

		,"_createDomElement":  function( domParent ) {
			var dateDisplayedShort = '<sub>'+this.date.toString('ddd').substr(0,1).toUpperCase()+'</sub>'+this.date.toString('dd/MM');
...
console.log('Edit ' + noteData.type+' '+noteData.title );
			...
			var noteDate  = new Date( parseInt(this.note.time,10) ).setTimezone( this.note.time.substring( this.note.time.indexOf('+') ) );
			var posTop    = this.domParent.offset().top;
			...
			this.dom.css('top', posTop+'px').css('height', posHeight+'px');
		...
		,"renderType": function( noteData, toolbarDom, editingDom ) {
			...
			bulletListDom.find('.bulletIcon')
				.on('click', function() {
					var oldType = $(this).siblings('.selected').attr('name');
					var newType = $(this).attr('name');
console.log( 'Update type ' + oldType + ' -> ' + newType );

					var selectedPos = $(this).siblings('.selected').offset();
					var clickedPos  = $(this).offset();

					// Reverse

					var bulletList = $( $('.bulletList .bulletIcon').toArray().reverse() );


					bulletList.each(
						function() {
							var pos=$(this).offset();
							$(this).css('position','fixed').offset(pos); //.css('left', (0.0+pos.left)+'px');
							});
					...
					setTimeout(
						function() {
							...
							if( nextThanClicked.length > 0 ) {
								...
								clickedElement.parent().append( oldSelectedElement );
								}
					...
					Provider.update( noteData.id, {"type": newType} );
					});
		...
		,"renderTitle": function( noteData, toolbarDom, editingDom ) {
			...
			SpanToInput.create(
				...
				,"onChange": function( newValue ) {
					Provider.update( noteData.id, { "title": newValue.trim() } );
			...
			var thisObj = this;
			toolbarDom
				.prepend($('<div class="button" name="delete"></div>').on(
					'click',
					function() {
						DailyListApp.persistence.deleteNote( noteData.id );
						thisObj.hide();
						}))
				.prepend($('<div class="button" name="back"></div>').on(
					'click',
					function() {
						thisObj.hide();
						})
		...
		,"renderDate": function( noteData, toolbarDom, editingDom ) {
			var noteDate = new Date( parseInt(noteData.time,10) ).setTimezone( noteData.time.substring( noteData.time.indexOf('+') ) );
			...
			SpanToInput.create(
				...
				,"onEmpty": function() {
					...
					var newDateValue = ""+newDate.getTime()+newDate.toString().substr(3+newDate.toString().indexOf("GMT"),1)+"GMT"+newDate.toString().substr(4+newDate.toString().indexOf("GMT"),2)+"";

					Provider.update( noteData.id, {"time": newDateValue} );
				...
				,"onChange": function( newValue ) {
					...
					if(    newValues.length != 3 
						|| isNaN(parseInt(newValues[0],10))
						|| isNaN(parseInt(newValues[1],10))
						|| isNaN(parseInt(newValues[2],10)) ) {
					...
					$(this).find('.category').each(function() {
						$(this)
							...
							.one(
								'click',
								function() {
									Provider.update( elem.id, {"lists": []} );
									$(this).parent().text(''+L('Editing.No grouped')+'.')
									});
						});
					});
		...
		,"show": function() {
			this.dom.css('top', '0px').css('height', '100%');
			this.dom.find('.toolbar').css('height', '1.66em');
			}
		,"hide": function() {
			this.dom.find('.toolbar').css('height', '0em');
...
console.log('Edited');
		...
		,"construct": function( spanDom, options ) {
			...
			this.dom = $(spanDom);
			this.dom = $( spanDom ).map( function(elem) { if( 1 != this.nodeType ) { return null; } return this; } );
...
console.log('Click on ');
					var inputDom = $();
					if( '' != thisObj.options.inputId ) { inputDom = thisObj.dom.siblings('input#'+thisObj.options.inputId); }
					if( 0 == inputDom.length ) {
						// Create <input>
						var width    = $(thisObj.dom).width();
						var oldValue = thisObj.options.inputValue;
						if( "" == oldValue ) { oldValue = thisObj.dom.get(0).innerHTML; }
						...
						inputDom
							.on( 'blur',
							function() {
								var newValue = inputDom.get(0).value;
						...
						$(thisObj.dom).after( inputDom );
						}

					$(thisObj.dom).hide();
					inputDom.trigger('focus').trigger('click');
					});
...
DailyListApp = {
	...
	,"getLiteral": function( key ) {
		...
		if( 'undefined' == typeof DailyListApp.i18n[lang] ) {
			...
			for( otherLang in DailyListApp.i18n ) {
				if( null != lang && otherLang.indexOf() == 0 ) { lang = otherLang; }
				}

			if( null == lang ) { lang = 'en-US'; }
			else               { DailyListApp.language = lang; }
			}
		...
		if( null == text || '' == text ) { text = '<span style="color:red;">'+key+'</span>'; }
	...
	,"init": function() {
		...
		if( 'undefined' != typeof $.mobile ) { $.mobile.page.prototype.options.disabled = true; }
        ...
        if( null == savedConfig ) { savedConfig = {}; }
...
DailyListApp.DiaryView = {
	...
	,"init": function() {
	    ...
	    $('.diary div.list').not('#background-list').remove();
	...
	,"attachItemEvents": function( noteItem ) {
		noteItem
			.on( 'click', function() {
				// Edit
				var noteId = noteItem.attr('id').replace('item-', '');
		...
		// Hacking height
		if( noteItem.find('.title').height()/noteItem.height() > 1.8 ) {
	...
	,"addDatePickerToTitle": function() {
        ...
        $('.header')
        	.on('click', function() { 
        		$('#datepicker').trigger('focus').trigger('click');
        		});
        ...
        $('.header').on(
        	...
        	function() {
				...
				DailyListApp.DiaryView.currentDay.setTimezoneOffset( new Date().toString().substr(new Date().toString().indexOf('GMT')+3, 3) );
	...
	,"showCurrentDay": function( animation ) {
		...
		if( 'undefined' != typeof this.cachedPages[ currentDayTimestamp] ) {
			...
			Provider.read(
				function(noteValues) {
					return ('undefined' != typeof noteValues.time
						&& !isNaN(parseInt( noteValues.time, 10 ))
						&& parseInt( noteValues.time, 10 ) >= currentDayTimestamp
						&& parseInt( noteValues.time, 10 ) < (currentDayTimestamp+86400000));
					},
				listItems );
...
//					&& 'undefined' != typeof noteValues.status && 'done' != noteValues.status );
	...
	,"renderNewItemForm": function() {
		...
		if( 0 == newItemDOM.length ) {
			...
			newItemDOM.find('input').attr('placeholder', L("Editing.Write down") );

			newItemDOM.find('#newTaskType').on(
				'click',
				function() {
					if( 'none' == $('.newTaskTypeToolbox').css('display') ) {
						$('.newTaskTypeToolbox').css('top',  ($('#newTaskType').position().top-$('.newTaskTypeToolbox').outerHeight()+11)+'px')
						$('.newTaskTypeToolbox').css('left', ($('#newTaskType').position().left-15)+'px');
						$('.newTaskTypeToolbox').show();
						}
					else {
						$('.newTaskTypeToolbox').hide();
						}
					});

			Note.TYPE_NAMES.forEach( function(typeName) { 
				newItemDOM.find('.newTaskTypeToolbox').append(
					DailyListApp.DiaryView.templates.newItemBullet.replace(/##TYPE##/g, typeName.toLowerCase()).replace(/##BULLET##/g, Note.TYPES[typeName].BULLET)
					);
				});
			

			newItemDOM.find('.newTaskTypeToolbox .bulletButton').on(
				'click',
				function( ev ) {
					var targetDOM = $(ev.target);
					var type = targetDOM.attr('name');
					var text = targetDOM.text().trim();

					$('#newTaskType').attr('name', type).get(0).innerHTML = text;

					$('.newTaskTypeToolbox').hide();

					newItemDOM.find('input').focus();
				});


			newItemDOM.find('input').on('keypress', function( event ) {
				if( 13 == event.keyCode ) { DailyListApp.persistence.createNote( DailyListApp.DiaryView.currentDay ); }
	        	});

			newItemDOM.find('input').on('blur', function( event ) {
				var currentValue = event.target.value.trim();
				if( '' != currentValue ) {
					setTimeout(
						function() {
							if( 'none' == $('.newTaskTypeToolbox').css('display') ) { DailyListApp.persistence.createNote( DailyListApp.DiaryView.currentDay ); }
							},
						100 );
					}
	        	});
		...
		pageDOM.find('.itemBackground').first().replaceWith( newItemDOM );
	...
	,"showNewItemForm": function() {
		if( $('#newItem').length>0 ) {
			if( '' != $('#newItem input').get(0).value.trim() ) {
				$('#newItem input').blur();
				}
			else {
				$('#newItem').remove();
			...
			newItemDOM.show();
			newItemDOM.find('input').trigger('focus').trigger('click');
			}
	...
	,"updateI18N": function() {
		...
		if( 0 == numNotes ) {
			var now = this.currentDay.clone();
				now.setTimeToNow();
			var time = ""+now.getTime()+now.toString().substr(3+now.toString().indexOf("GMT"),1)+"GMT"+now.toString().substr(4+now.toString().indexOf("GMT"),2)+"";
...
DailyListApp.Menu = {
	...
	,"showMenu": function() {
		if( !this.isShownAppTitle() ) { this.showAppTitle(); }
		if(  this.isShownMenu() ) { return; }

		// Show menu
		$('.menu .menu-icon div').addClass('selected');
		$('.menu .background').css('display', 'block').css('opacity', 0.6);
		$('.menu .panel').css('display', 'block').css('width', ($('body').width()/3)+'px' );
		setTimeout( function() {
			$('.menu .panel').css('width', '33%');
			}, 410 );

		$('.menu .panel > div').get(0).innerHTML = L('Menu.Lists');
		$('.menu .lists').append(
	...
	,"hideAppTitle": function() {
		if( !this.isShownAppTitle() ) { return; }
		if( this.isShownMenu() ) { this.hideMenu(); }
	...
	,"hideMenu": function() {
		...
		// Hide menu
		$('.menu .menu-icon div').removeClass('selected');
		$('.menu .background').css('opacity', 0.0);
		$('.menu .panel').css('width', ($('body').width()/3)+'px' ).css('width', '0px');
		setTimeout( function() {
	...
	,"toggleAppTitle": function() {
		if( this.isShownAppTitle() ) { this.hideAppTitle(); } else { this.showAppTitle(); }
		}    //  END method DailyListApp.Menu.toggleAppTitle()

	,"toggleMenu": function() {
		if( this.isShownMenu() ) { this.hideMenu(); } else { this.showMenu(); }
...
DailyListApp.persistence = {
	 "createNote": function( currentDay ) {
		...
		values.title = $('#newItem input').get(0).value;
		values.type  = $('#newTaskType').attr('name');
		values.time  = ""+now.getTime()+now.toString().substr(3+now.toString().indexOf("GMT"),1)+"GMT"+now.toString().substr(4+now.toString().indexOf("GMT"),2)+"";

		values.title = values.title.trim();

		if( '' != values.title ) {
			var separator='';
			if( ['.',' - '].some(function(sep) { if(values.title.indexOf(sep)>=0){ separator=sep; return true;} }) ) {
